...
# Cosmos Hub mainnet validator monitor.
# Uses the shared $HOME/.validator-monitor.env pattern.

[ -f "$HOME/.validator-monitor.env" ] && source "$HOME/.validator-monitor.env"

DISCORD_WEBHOOK="${COSMOS_MAINNET_DISCORD_WEBHOOK_URL:-${COSMOS_DISCORD_WEBHOOK_URL:-${DISCORD_WEBHOOK:-REPLACE_WITH_DISCORD_WEBHOOK}}}"
STATE_FILE="${COSMOS_MAINNET_STATE_FILE:-$HOME/cosmos_mainnet_status.txt}"
LOG_FILE="${COSMOS_MAINNET_LOG_FILE:-$HOME/cosmos_mainnet_monitor.log}"

CHAIN_ID="${COSMOS_MAINNET_CHAIN_ID:-cosmoshub-4}"
SERVICE_NAME="${COSMOS_MAINNET_SERVICE:-gaiad-mainnet}"
GAIAD_BIN="${COSMOS_MAINNET_BIN:-$HOME/go/bin/gaiad-mainnet}"
GAIA_HOME="${COSMOS_MAINNET_HOME:-$HOME/.gaia-mainnet}"
RPC_URL="${COSMOS_MAINNET_RPC:-http://127.0.0.1:26657}"
NODE_URL="${COSMOS_MAINNET_NODE:-tcp://127.0.0.1:26657}"
QUERY_NODE="${COSMOS_MAINNET_QUERY_NODE:-$NODE_URL}"
DATA_PATH="${COSMOS_MAINNET_DATA_PATH:-$GAIA_HOME/data}"

VALIDATOR_OPERATOR="${COSMOS_MAINNET_VALOPER:-REPLACE_WITH_MAINNET_VALOPER}"
OPERATOR_ADDRESS="${COSMOS_MAINNET_OPERATOR_ADDRESS:-REPLACE_WITH_MAINNET_OPERATOR_ADDRESS}"
LEDGER_ADDRESS="${COSMOS_MAINNET_LEDGER_ADDRESS:-REPLACE_WITH_MAINNET_PAYMENT_ADDRESS}"
MIN_PEERS="${COSMOS_MAINNET_MIN_PEERS:-5}"
STUCK_CHECKS="${COSMOS_MAINNET_STUCK_CHECKS:-6}"
LOW_BALANCE_UATOM="${COSMOS_MAINNET_LOW_BALANCE_UATOM:-1000000}"
MISSED_BLOCK_WARN_INCREASE="${COSMOS_MAINNET_WARN_MISSED_INCREASE:-true}"
MAX_MISSED_BLOCKS="${COSMOS_MAINNET_MAX_MISSED_BLOCKS:-10}"
...
if [ ! -x "$GAIAD_BIN" ] && ! command -v "$GAIAD_BIN" > /dev/null 2>&1; then
    if [ "$PREV_STATE" != "BINARY_MISSING" ]; then
        FIELDS="[{\"name\":\"Binary\",\"value\":\"${GAIAD_BIN}\",\"inline\":false},{\"name\":\"Action\",\"value\":\"Check binary path or COSMOS_MAINNET_BIN\",\"inline\":false}]"
        send_discord "Cosmos Mainnet Binary Missing" "gaiad-mainnet binary was not found on ${HOSTNAME}" "$COLOR_RED" "$FIELDS"
...
if ! service_running "$SERVICE_NAME" && ! pgrep -x "$(basename "$GAIAD_BIN")" > /dev/null 2>&1; then
    if [ "$PREV_STATE" != "NODE_DOWN" ]; then
        ...
        send_discord "Cosmos Mainnet Node Down" "Cosmos Hub mainnet service is not running on ${HOSTNAME}" "$COLOR_RED" "$FIELDS"
...
if [ -z "$STATUS_JSON" ] || ! echo "$STATUS_JSON" | jq -e '.result.sync_info' > /dev/null 2>&1; then
    if [ "$PREV_STATE" != "RPC_DOWN" ]; then
        ...
        send_discord "Cosmos Mainnet RPC Issue" "Mainnet service is running but local CometBFT RPC is not responding" "$COLOR_YELLOW" "$FIELDS"
...
if [ "$NODE_CHAIN_ID" != "$CHAIN_ID" ]; then
    if [ "$PREV_STATE" != "CHAIN_MISMATCH" ]; then
        ...
        send_discord "Cosmos Mainnet Chain Mismatch" "Local mainnet RPC reports the wrong chain ID" "$COLOR_RED" "$FIELDS"
...
VALCONS="${COSMOS_MAINNET_VALCONS:-}"
if [ -z "$VALCONS" ]; then
    VALCONS=$("$GAIAD_BIN" tendermint show-validator --home "$GAIA_HOME" 2>/dev/null)
fi
...
if [ -n "$VALCONS" ]; then
    ...
    MISSED_BLOCKS=$(echo "$SIGNING_JSON" | jq -r 'if (.val_signing_info or .address) then (.missed_blocks_counter // .val_signing_info.missed_blocks_counter // "0") else "unknown" end' 2>/dev/null)
    TOMBSTONED=$(echo "$SIGNING_JSON" | jq -r 'if (.val_signing_info or .address) then (.tombstoned // .val_signing_info.tombstoned // "false") else "unknown" end' 2>/dev/null)
fi

OPERATOR_BALANCE_UATOM=$(get_balance_uatom "$OPERATOR_ADDRESS")
[ -z "$OPERATOR_BALANCE_UATOM" ] && OPERATOR_BALANCE_UATOM="0"
OPERATOR_BALANCE=$(format_atom "$OPERATOR_BALANCE_UATOM")
SYSTEM_STATS=$(get_system_stats)
...
if [ "$TOMBSTONED" = "true" ]; then
    if [ "$PREV_STATE" != "TOMBSTONED" ]; then
        ...
        send_discord "Cosmos Mainnet Validator Tombstoned" "Cosmos Hub mainnet validator is tombstoned" "$COLOR_RED" "$FIELDS"
...
if [ "$VAL_JAILED" = "true" ]; then
    if [ "$PREV_STATE" != "JAILED" ]; then
        ...
        send_discord "Cosmos Mainnet Validator Jailed" "Cosmos Hub mainnet validator is jailed" "$COLOR_RED" "$FIELDS"
...
if [ "$VAL_STATUS" = "unknown" ]; then
    if [ "$PREV_STATE" != "VALIDATOR_UNKNOWN" ]; then
        ...
        send_discord "Cosmos Mainnet Validator Query Issue" "Monitor could not confirm mainnet validator state" "$COLOR_YELLOW" "$FIELDS"
    ...
    if [ "$PREV_STATE" != "NOT_BONDED" ]; then
        ...
        send_discord "Cosmos Mainnet Validator Not Bonded" "Cosmos Hub mainnet validator is not bonded" "$COLOR_YELLOW" "$FIELDS"
    ...
    if [ "$PREV_STATE" != "SYNCING" ]; then
        ...
        send_discord "Cosmos Mainnet Node Syncing" "Cosmos Hub mainnet node is catching up" "$COLOR_YELLOW" "$FIELDS"
    ...
    if [ "$PREV_STATE" != "LOW_PEERS" ]; then
        ...
        send_discord "Cosmos Mainnet Low Peers" "Cosmos Hub mainnet peer count is below threshold" "$COLOR_YELLOW" "$FIELDS"
    ...
    if [ "$PREV_STATE" != "ACTIVE" ]; then
        ...
        FIELDS="[{\"name\":\"Node\",\"value\":\"${NODE_MONIKER}\",\"inline\":true},{\"name\":\"Chain\",\"value\":\"${NODE_CHAIN_ID}\",\"inline\":true},{\"name\":\"Height\",\"value\":\"${HEIGHT}\",\"inline\":true},{\"name\":\"Peers\",\"value\":\"${PEERS}\",\"inline\":true},{\"name\":\"Validator\",\"value\":\"${VAL_STATUS}\",\"inline\":true},{\"name\":\"Jailed\",\"value\":\"${VAL_JAILED}\",\"inline\":true},{\"name\":\"Missed Blocks\",\"value\":\"${MISSED_BLOCKS}\",\"inline\":true},{\"name\":\"Operator Balance\",\"value\":\"${OPERATOR_BALANCE}\",\"inline\":true},{\"name\":\"Version\",\"value\":\"${VERSION}\",\"inline\":false},{\"name\":\"System\",\"value\":\"${SYSTEM_STATS}\",\"inline\":false}]"
        send_discord "Cosmos Mainnet Validator Active" "Cosmos Hub mainnet monitor is healthy on ${HOSTNAME}" "$COLOR_GREEN" "$FIELDS"
        echo "ACTIVE" > "$STATE_FILE"
    fi
    log_line "ACTIVE - Height: $HEIGHT | Peers: $PEERS | Validator: $VAL_STATUS | Jailed: $VAL_JAILED | Missed: $MISSED_BLOCKS | Balance: $OPERATOR_BALANCE"
fi

HEIGHT_FILE="/tmp/cosmos_mainnet_last_height"
STUCK_FILE="/tmp/cosmos_mainnet_stuck_count"
if is_number "$HEIGHT" && [ "$HEIGHT" != "0" ] && [ -f "$HEIGHT_FILE" ]; then
    ...
    if [ "$HEIGHT" = "$LAST_HEIGHT" ] && [ "$CATCHING_UP" != "true" ]; then
        ...
        if [ "$STUCK_COUNT" -ge "$STUCK_CHECKS" ]; then
            ...
            send_discord "Cosmos Mainnet Height Stuck" "Mainnet block height has not advanced across repeated checks" "$COLOR_RED" "$FIELDS"
            echo "0" > "$STUCK_FILE"
...
MISSED_FILE="/tmp/cosmos_mainnet_missed_blocks"
if [ "$MISSED_BLOCK_WARN_INCREASE" = "true" ] && is_number "$MISSED_BLOCKS"; then
    if [ -f "$MISSED_FILE" ]; then
        ...
        if is_number "$LAST_MISSED" && [ "$MISSED_BLOCKS" -gt "$LAST_MISSED" ]; then
            FIELDS="[{\"name\":\"Previous\",\"value\":\"${LAST_MISSED}\",\"inline\":true},{\"name\":\"Current\",\"value\":\"${MISSED_BLOCKS}\",\"inline\":true},{\"name\":\"Height\",\"value\":\"${HEIGHT}\",\"inline\":true},{\"name\":\"System\",\"value\":\"${SYSTEM_STATS}\",\"inline\":false}]"
            send_discord "Cosmos Mainnet Missed Blocks Increased" "Mainnet validator missed block counter increased" "$COLOR_YELLOW" "$FIELDS"
...
HOUR_FILE="/tmp/cosmos_mainnet_resource_alert_$(date +%Y%m%d%H)"
if [ ! -f "$HOUR_FILE" ]; then
    ...
    elif is_number "$OPERATOR_BALANCE_UATOM" && [ "$OPERATOR_BALANCE_UATOM" -lt "$LOW_BALANCE_UATOM" ]; then
        WARNING="Low operator hot-wallet balance: ${OPERATOR_BALANCE}"
    ...
    if [ -n "$WARNING" ]; then
        ...
        send_discord "Cosmos Mainnet Resource Warning" "Attention needed on ${HOSTNAME}" "$COLOR_YELLOW" "$FIELDS"
        touch "$HOUR_FILE"
    fi
fi

HOUR=$(date +%H)
DAILY_FILE="/tmp/cosmos_mainnet_daily_$(date +%Y%m%d)"
if [ "$HOUR" = "09" ] && [ ! -f "$DAILY_FILE" ]; then
    FIELDS="[{\"name\":\"Node\",\"value\":\"${NODE_MONIKER}\",\"inline\":true},{\"name\":\"Chain\",\"value\":\"${NODE_CHAIN_ID}\",\"inline\":true},{\"name\":\"Syncing\",\"value\":\"${CATCHING_UP}\",\"inline\":true},{\"name\":\"Height\",\"value\":\"${HEIGHT}\",\"inline\":true},{\"name\":\"Peers\",\"value\":\"${PEERS}\",\"inline\":true},{\"name\":\"Validator\",\"value\":\"${VAL_STATUS}\",\"inline\":true},{\"name\":\"Jailed\",\"value\":\"${VAL_JAILED}\",\"inline\":true},{\"name\":\"Tokens\",\"value\":\"$(format_atom "$VAL_TOKENS")\",\"inline\":true},{\"name\":\"Commission\",\"value\":\"${COMMISSION_RATE}\",\"inline\":true},{\"name\":\"Missed Blocks\",\"value\":\"${MISSED_BLOCKS}\",\"inline\":true},{\"name\":\"Operator Balance\",\"value\":\"${OPERATOR_BALANCE}\",\"inline\":true},{\"name\":\"System\",\"value\":\"${SYSTEM_STATS}\",\"inline\":false}]"
    send_discord "Cosmos Mainnet Daily Report" "${HOSTNAME} Cosmos Hub mainnet daily health summary" "$COLOR_BLUE" "$FIELDS"
    touch "$DAILY_FILE"
fi

find /tmp -name "cosmos_mainnet_resource_alert_*" -mtime +1 -delete 2>/dev/null
find /tmp -name "cosmos_mainnet_daily_*" -mtime +1 -delete 2>/dev/null
