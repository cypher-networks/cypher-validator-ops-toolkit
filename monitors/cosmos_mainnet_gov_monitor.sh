...
# Cosmos Hub mainnet governance proposal/vote monitor.
...
DISCORD_WEBHOOK="${COSMOS_MAINNET_GOV_DISCORD_WEBHOOK_URL:-${COSMOS_MAINNET_DISCORD_WEBHOOK_URL:-${COSMOS_DISCORD_WEBHOOK_URL:-${DISCORD_WEBHOOK:-REPLACE_WITH_DISCORD_WEBHOOK}}}}"
STATE_FILE="${COSMOS_MAINNET_GOV_STATE_FILE:-$HOME/cosmos_mainnet_gov_status.txt}"
LOG_FILE="${COSMOS_MAINNET_GOV_LOG_FILE:-$HOME/cosmos_mainnet_gov_monitor.log}"
...
VOTER_ADDRESS="${COSMOS_MAINNET_GOV_VOTER_ADDRESS:-${COSMOS_MAINNET_OPERATOR_ADDRESS:-REPLACE_WITH_MAINNET_OPERATOR_ADDRESS}}"
KEY_NAME="${COSMOS_MAINNET_GOV_KEY_NAME:-YOUR_MAINNET_KEY_NAME}"
GAS_PRICES="${COSMOS_MAINNET_GOV_GAS_PRICES:-0.005uatom}"
REMINDER_SECONDS="${COSMOS_MAINNET_GOV_REMINDER_SECONDS:-21600}"
...
ALERT_DIR="/tmp/cosmos_mainnet_gov_alerts"
mkdir -p "$ALERT_DIR"
...
if [ ! -x "$GAIAD_BIN" ] && ! command -v "$GAIAD_BIN" > /dev/null 2>&1; then
    send_discord "Cosmos Mainnet Gov Monitor Error" "gaiad-mainnet binary was not found" "$COLOR_RED" "[{\"name\":\"Binary\",\"value\":\"${GAIAD_BIN}\",\"inline\":false}]"
...
if [ -z "$PROPOSALS_JSON" ] || ! echo "$PROPOSALS_JSON" | jq -e '.proposals' > /dev/null 2>&1; then
    ...
    if [ "$PREV_STATE" != "QUERY_FAILED" ]; then
        ...
        send_discord "Cosmos Mainnet Gov Query Failed" "Could not query active governance proposals" "$COLOR_YELLOW" "$FIELDS"
...
if [ "$OPEN_COUNT" -eq 0 ]; then
    ...
    if [ "$PREV_STATE" != "NO_ACTIVE_PROPOSALS" ]; then
        ...
        send_discord "Cosmos Mainnet Gov Clear" "No active voting-period proposals" "$COLOR_GREEN" "$FIELDS"
...
for proposal_id in "${PROPOSAL_IDS[@]}"; do
    ...
    if [ -n "$VOTE" ] && [ "$VOTE" != "null" ]; then
        ...
        if [ -f "${ALERT_DIR}/proposal_${proposal_id}.last" ] && [ ! -f "${ALERT_DIR}/proposal_${proposal_id}.voted" ]; then
            ...
            send_discord "Cosmos Mainnet Gov Vote Recorded" "Configured voter has voted on proposal #${proposal_id}" "$COLOR_GREEN" "$FIELDS"
        ...
        if should_remind "$proposal_id"; then
            ...
            send_discord "Cosmos Mainnet Gov Vote Needed" "Active proposal has no recorded vote from configured voter" "$COLOR_YELLOW" "$FIELDS"
        fi
    fi
done
...
HOUR=$(date +%H)
DAILY_FILE="/tmp/cosmos_mainnet_gov_daily_$(date +%Y%m%d)"
if [ "$HOUR" = "09" ] && [ ! -f "$DAILY_FILE" ]; then
    ...
    send_discord "Cosmos Mainnet Gov Daily Report" "${HOSTNAME} Cosmos Hub governance summary" "$COLOR_BLUE" "$FIELDS"
    touch "$DAILY_FILE"
fi

find "$ALERT_DIR" -type f -mtime +14 -delete 2>/dev/null
find /tmp -name "cosmos_mainnet_gov_daily_*" -mtime +2 -delete 2>/dev/null
